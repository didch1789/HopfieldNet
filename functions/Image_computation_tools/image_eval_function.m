...
% ..
    ...
    out1 = []; out2 = []; out3 = []; out4 = []; out5 = []; out6 = []; out7 = []; out8 = []; out9 = []; out10 = []; %#ok<*NASGU>
    ...
    function setup_inputs(nout)
        ...
        if isempty(mask)
            error('mask is empty. no-mask opt. not implemented yet.');
        else
            fprintf('Found mask: %s\n', mask);
        end
    ...
    function build_eval_string(numargs)
        fstr = '[';
        for arg = 1:numargs
            fstr = [fstr 'out' num2str(arg)]; %#ok<*AGROW>
            if arg ~= numargs
                fstr = [fstr ', '];
            else
...
% % -------------------------------------------------------------------
 ...
 function [outimagenames, nimgs_this_output] = define_output_names(outimagelabels, nout, outputs)
        
        for i = 1:nout
            if length(outimagelabels) >= i  % if we have an output image name for this output
                outnm = outimagelabels{i};

                % Make sure we have .img or .nii extensions
